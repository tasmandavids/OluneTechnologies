...
export type UpcomingBillsResult = {
  connected: boolean;
  configured: boolean;
  bills: UpcomingBill[];
  fetchError: string | null;
...
/** Supplier bills (Xero ACCPAY) the studio owes, soonest due date first. */
export async function fetchUpcomingBills(
  ...
  const loaded = await loadStudioXeroClient(supabase, studioId, redirectUri);
  if (!loaded) {
    return { connected: false, configured, bills: [], fetchError: null };
  }

  try {
    const res = await loaded.client.accountingApi.getInvoices(
      loaded.tenantId,
      undefined,
    ...
    const today = new Date().toISOString().slice(0, 10);
    const bills: UpcomingBill[] = (res.body.invoices ?? []).map((inv) => {
      const dueDate = inv.dueDate ? String(inv.dueDate).slice(0, 10) : null;
      return {
        id: inv.invoiceID ?? inv.invoiceNumber ?? `bill-${Math.random().toString(36).slice(2)}`,
        reference: inv.invoiceNumber ?? inv.reference ?? "Bill",
    ...
    return { connected: true, configured, bills, fetchError: null };
  } catch (e) {
    return { connected: true, configured, bills: [], fetchError: xeroErrorMessage(e) };
  }
}
