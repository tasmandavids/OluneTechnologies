...
--  Pinned `to authenticated` per 0123. Both predicates are gated on
--  private.current_studio() / private.is_platform_operator(), so neither can
--  match an anonymous request; naming the role keeps the policy off anon,
--  authenticator and dashboard_user, and out of the multiple_permissive_policies
--  lint that 0123 swept the schema to clear. A policy created without a `TO`
--  clause is attached to PUBLIC and lands back in that lint once per role.
...
create policy "audit_events_operator_read" on public.audit_events
  for select to authenticated using (private.is_platform_operator());
